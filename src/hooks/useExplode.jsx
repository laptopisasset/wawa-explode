...
export const useExplode = (group, { distance = 3, enableRotation = true }) => {
  ...
  useEffect(() => {
    ...
    group.current.children.forEach((mesh) => {
      mesh.targetPostion = mesh.originalPosition
        .clone()
  ...
  useFrame(() => {
    group.current.children.forEach((mesh) => {
      ...
      mesh.position.x = THREE.MathUtils.lerp(
        ...
        mesh.targetPostion.x,
        scrollData.offset // 0 when no scroll and 1 when scroll
      );
      mesh.position.y = THREE.MathUtils.lerp(
        mesh.originalPosition.y,
        mesh.targetPostion.y,
        scrollData.offset // 0 when no scroll and 1 when scroll
      );
      mesh.position.z = THREE.MathUtils.lerp(
        mesh.originalPosition.z,
        mesh.targetPostion.z,
        scrollData.offset // 0 when no scroll and 1 when scroll
      );
